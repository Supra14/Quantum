...
def prepare_qubits(alice, length):
    ...
    for i in range(length):
        q = Qubit(alice)  # Create a new qubit for each bit
        
        # Apply X gate if bit is 1
        if alice_bits[i] == 1:
            q.X()
        
        # Apply Hadamard gate to simulate the effect of a controlled Hadamard
        # We use the fact that:
        # 1. H|0⟩ = (|0⟩ + |1⟩) / sqrt(2)
        # 2. H|1⟩ = (|0⟩ - |1⟩) / sqrt(2)
        # Simulate a controlled Hadamard gate:
        # If basis is diagonal (alice_bases[i] == 1), apply Hadamard gate
...
def measure_qubits(bob, qubits, length):
    ...
    for i in range(length):
        ...
        # Apply Hadamard gate if Bob uses diagonal basis
        if bob_bases[i] == 1:
            q.H()
